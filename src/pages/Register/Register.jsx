...
const Register = () => {
    ...
    const onSubmit = (data) => {
        ...
        if(password.length < 6){
            setUserError('Password must be at least 6 characters or longer.')
            toast.error(userError)
            return;
        }
        if(!/^(?=.*[a-z])(?=.*[A-Z]).+$/.test(password)){
            setUserError("Your password should have at least one upperCase and one lowerCase characters");
            toast.error(userError)
            return;
        }
    ...
    return (
        <div>
            ...
            <div className="w-full max-w-lg p-10  space-y-3 rounded-xl dark:text-gray-800 bg-base-200 absolute top-[60%] left-[50%] translate-x-[-50%] translate-y-[-50%] shadow-2xl">
